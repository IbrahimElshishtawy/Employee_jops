...
export class AttendanceController {
  ...
  @Post('check-in')
  @ApiOperation({ summary: 'Register employee check-in with GPS evidence & security signals' })
  checkIn(@CurrentUser('id') userId: string, @Body() dto: CheckInDto) {
    return this.attendanceService.checkIn(userId, dto);
  }

  @Post('check-out')
  @ApiOperation({ summary: 'Register employee check-out with GPS evidence & duration calculations' })
  checkOut(@CurrentUser('id') userId: string, @Body() dto: CheckOutDto) {
    return this.attendanceService.checkOut(userId, dto);
  }

  @Get('today')
  @ApiOperation({ summary: 'Get current authenticated employee attendance status for today' })
  getToday(@CurrentUser('id') userId: string) {
    return this.attendanceService.getTodayStatus(userId);
  }

  @Get('me')
  @ApiOperation({ summary: 'Get personal attendance history for current employee (supports month/date filters)' })
  getMyAttendance(
  ...
  @ApiOperation({ summary: 'HR Manual Attendance Adjustment / Creation with mandatory reason' })
  manualAdjustment(
  ...
  @ApiOperation({ summary: 'HR Query: Get specific employee attendance records' })
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER, Role.SUPERVISOR)
  @ApiOperation({ summary: 'HR Query: Get attendance records for a specific workplace/branch' })
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER, Role.SUPERVISOR)
  @ApiOperation({ summary: 'HR Query: Get attendance records for a specific department' })
  ...
  @Get('records')
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER, Role.SUPERVISOR)
  @ApiOperation({ summary: 'HR General Query: List attendance records across company' })
