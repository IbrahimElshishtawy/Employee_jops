...
describe('Employee Core — 10 Mandatory E2E Scenarios', () => {
  ...
  const mockPrismaService: any = {
    ...
    attendanceRecord: {
      ...
      update: jest.fn(({ where, data }) => {
        const record = Object.values(attendanceRecords).find((r) => r.id === where.id);
        if (record) {
  ...
  it('Scenario 2: Existing Employee Google Login -> ACTIVE_EMPLOYEE', async () => {
    const loginRes = await authService.googleLogin({
      idToken: 'test-google-token:employee.active@example.test:google-active-id',
    });
  ...
  it('Scenario 3: Suspended Employee -> Access Denied (ForbiddenException)', async () => {
    await expect(
      authService.googleLogin({
        idToken: 'test-google-token:employee.suspended@example.test:google-suspended-id',
      }),
