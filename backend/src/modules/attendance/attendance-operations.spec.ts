...
describe('Phase 03 — Attendance & Workforce Operations (23 Scenarios)', () => {
  ...
  const mockPrismaService: any = {
    ...
    employeeProfile: {
      findUnique: jest.fn(({ where }) => {
        const found = Object.values(mockUsers).find((u) => u.employeeProfile?.id === where.id);
        if (found) {
    ...
    attendanceRecord: {
      ...
      update: jest.fn(({ where, data }) => {
        const record = Object.values(recordsStore).find((r) => r.id === where.id);
        if (record) {
  ...
  it('12 & 13. Arrival status and lateMinutes evaluated based on schedule grace period', async () => {
    ...
    expect([AttendanceStatus.PRESENT, AttendanceStatus.LATE]).toContain(record.status);
  ...
  it('19. Employee personal attendance: queries only authenticated user records', async () => {
    const result = await attendanceService.getMyAttendance('user-active', { page: 1, limit: 10 });
  ...
  it('20. HR attendance access: retrieves records by employee, workplace, and department', async () => {
    const empResult = await attendanceService.getEmployeeAttendance('prof-active', {});
    const wpResult = await attendanceService.getWorkplaceAttendance('wp-hq-01', {});
    const deptResult = await attendanceService.getDepartmentAttendance('Engineering', {});
  ...
  it('21. HR Manual Attendance Correction: applies adjustment, records reason, and logs audit', async () => {
    ...
    expect(adjusted.manualCorrectionReason).toBe('Biometric reader network glitch confirmed by IT');
