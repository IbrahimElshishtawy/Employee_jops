...
describe('Phase 09 — Security Hardening, E2E & Concurrency Suite', () => {
  ...
  describe('Authentication Hardening & Token Rotation', () => {
    ...
    it('should detect Refresh Token Replay and invalidate ALL user sessions', async () => {
      ...
      await expect(authService.refreshToken('replayed_revoked_token')).rejects.toThrow(
    ...
    it('should reject expired refresh tokens', async () => {
      ...
      await expect(authService.refreshToken('expired_token_xyz')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  });
  ...
  describe('IDOR & Data Access Isolation', () => {
    ...
    it('should ALLOW Employee A to access their OWN salary profile', async () => {
      ...
      const result = await payrollService.getSalaryProfile('emp-profile-a', currentUser);
    ...
    it('should ALLOW HR_ADMIN to access ANY employee salary profile', async () => {
      ...
      mockPrisma.salaryProfile.findUnique.mockResolvedValue(employeeBSalaryProfile);

      const result = await payrollService.getSalaryProfile('emp-profile-b', hrUser);
      expect(result).toBeDefined();
  ...
  describe('Concurrency & State Consistency', () => {
    it('should execute concurrent actions cleanly through database transactions', async () => {
      const operations = [
        mockPrisma.$transaction(async (tx: any) => tx.user.findUnique({ where: { id: '1' } })),
        mockPrisma.$transaction(async (tx: any) => tx.user.findUnique({ where: { id: '2' } })),
      ];
