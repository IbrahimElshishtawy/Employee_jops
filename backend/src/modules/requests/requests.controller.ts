...
export class RequestsController {
  ...
  @Post()
  @ApiOperation({
    summary: 'Submit a new request (Leave, Absence, Permission, Late Excuse, Early Leave, Half Day, etc.)',
  })
  @ApiResponse({ status: 201, description: 'Request created successfully and pending review' })
  @ApiResponse({ status: 400, description: 'Validation error or insufficient leave balance' })
  ...
  @Get('me')
  @ApiOperation({ summary: 'Get paginated submitted requests for current employee' })
  getMyRequests(
  ...
  @Get('my-requests')
  @ApiOperation({ summary: 'Alias: Get submitted requests for current employee' })
  getMyRequestsAlias(
    @CurrentUser('employeeProfileId') employeeProfileId: string,
    @Query() query: QueryRequestsDto,
  ) {
  ...
  @ApiOperation({ summary: 'Get current year leave balances and remaining days for current employee' })
  ...
  @ApiOperation({ summary: 'HR: Allocate / Initialize new leave balance for an employee' })
  createLeaveBalance(
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER)
  @ApiOperation({ summary: 'HR: Adjust total or used days on an existing leave balance' })
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER, Role.SUPERVISOR)
  @ApiOperation({
    summary: 'HR: Filtered and paginated queue of employee requests (status, department, workplace, dates)',
  })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get request details with approval history (IDOR protected: Owner or HR)' })
  @ApiParam({ name: 'id', description: 'Request UUID' })
  findOne(
    @Param('id') id: string,
    @CurrentUser() currentUser: { id: string; role: Role; employeeProfileId?: string },
  ) {
  ...
  @Patch(':id/cancel')
  @ApiOperation({ summary: 'Employee: Cancel a pending request (PATCH method)' })
  @ApiParam({ name: 'id', description: 'Request UUID' })
  cancelPatch(
    @Param('id') id: string,
    @CurrentUser() currentUser: { id: string; role: Role; employeeProfileId?: string },
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER, Role.SUPERVISOR)
  @ApiOperation({
    summary: 'HR: Approve an employee request (updates leave balance, attendance records, audit & notification)',
  })
  @ApiParam({ name: 'id', description: 'Request UUID' })
  approvePost(
    @Param('id') id: string,
  ...
  @HttpCode(HttpStatus.OK)
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER, Role.SUPERVISOR)
  @ApiOperation({ summary: 'HR: Reject an employee request with mandatory reason' })
  @ApiParam({ name: 'id', description: 'Request UUID' })
  rejectPost(
    @Param('id') id: string,
    @CurrentUser('id') approverId: string,
  ...
  @Patch(':id/reject')
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER, Role.SUPERVISOR)
  @ApiOperation({ summary: 'HR: Reject an employee request with mandatory reason (PATCH method)' })
  @ApiParam({ name: 'id', description: 'Request UUID' })
  rejectPatch(
    @Param('id') id: string,
    @CurrentUser('id') approverId: string,
