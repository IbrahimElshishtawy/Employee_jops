...
describe('Phase 04 — Employee Requests & Leave Management (30 Mandatory Scenarios)', () => {
  ...
  beforeEach(async () => {
    ...
    prismaService = {
      ...
      request: {
        findUnique: jest.fn(({ where }) => {
          if (where.id) {
            ...
            const user = Object.values(mockUsers).find((u) => u.employeeProfile?.id === req.employeeId);
          ...
          if (where.idempotencyKey) {
            return Promise.resolve(mockRequests.find((r) => r.idempotencyKey === where.idempotencyKey) || null);
          }
          ...
          return Promise.resolve(
            mockRequests.find(
              (r) =>
                ...
                r.startDate <= where.endDate?.gte &&
                r.endDate >= where.startDate?.lte,
            ) || null,
          );
        }),
        findMany: jest.fn(({ where, skip = 0, take = 10 }) => {
          let filtered = [...mockRequests];
          if (where.employeeId) filtered = filtered.filter((r) => r.employeeId === where.employeeId);
          if (where.status) filtered = filtered.filter((r) => r.status === where.status);
          if (where.type) filtered = filtered.filter((r) => r.type === where.type);
      ...
      leaveBalance: {
        findUnique: jest.fn(({ where }) => {
          if (where.id) {
            return Promise.resolve(mockLeaveBalances.find((b) => b.id === where.id) || null);
          }
          if (where.employeeId_leaveType_year) {
            const { employeeId, leaveType, year } = where.employeeId_leaveType_year;
            return Promise.resolve(
        ...
        findMany: jest.fn(({ where }) => {
          ...
          if (where.employeeId) filtered = filtered.filter((b) => b.employeeId === where.employeeId);
          if (where.year) filtered = filtered.filter((b) => b.year === where.year);
        ...
        upsert: jest.fn(({ where, update, create }) => {
          const { employeeId, leaveType, year } = where.employeeId_leaveType_year;
          let existing = mockLeaveBalances.find(
            (b) => b.employeeId === employeeId && b.leaveType === leaveType && b.year === year,
          );
          if (existing) {
            if (update.usedDays?.increment) existing.usedDays += update.usedDays.increment;
            if (update.remainingDays?.decrement) existing.remainingDays -= update.remainingDays.decrement;
            return Promise.resolve(existing);
          } else {
            const newBal = { id: `bal-${mockLeaveBalances.length + 1}`, ...create };
            mockLeaveBalances.push(newBal);
            return Promise.resolve(newBal);
          }
        }),
          ...
          return Promise.resolve(
            mockAttendanceRecords.find(
              (a) =>
                ...
                a.date?.toISOString() === where.employeeId_date?.date?.toISOString(),
            ) || null,
          );
        }),
        upsert: jest.fn(({ where, update, create }) => {
          ...
          if (index !== -1) {
            mockAttendanceRecords[index] = { ...mockAttendanceRecords[index], ...update };
            return Promise.resolve(mockAttendanceRecords[index]);
          } else {
            const newAtt = { id: `att-${mockAttendanceRecords.length + 1}`, ...create };
            mockAttendanceRecords.push(newAtt);
            return Promise.resolve(newAtt);
          }
        }),
        update: jest.fn(({ where, data }) => {
          const index = mockAttendanceRecords.findIndex((a) => a.id === where.id);
          if (index !== -1) {
            mockAttendanceRecords[index] = { ...mockAttendanceRecords[index], ...data };
            return Promise.resolve(mockAttendanceRecords[index]);
          }
          return Promise.resolve(null);
        }),
      ...
      auditLog: {
        create: jest.fn(({ data }) => {
          const newLog = { id: `audit-${mockAuditLogs.length + 1}`, ...data, createdAt: new Date() };
    ...
    notificationsService = {
      sendNotification: jest.fn((userId, title, body, type, data) => {
        const notif = { id: `notif-${mockNotifications.length + 1}`, userId, title, body, type, data };
  ...
  it('10. HR approves pending request successfully', async () => {
    ...
    expect(mockAuditLogs.some((l) => l.action === AuditAction.REQUEST_APPROVED)).toBe(true);
  ...
  it('11. HR rejects pending request with reason', async () => {
    ...
    expect(mockAuditLogs.some((l) => l.action === AuditAction.REQUEST_REJECTED)).toBe(true);
  });
  ...
  it('13. Employee cancels own pending request', async () => {
    ...
    const cancelled = await requestsService.cancel(
      req.id,
      { id: 'user-emp-1', employeeProfileId: 'emp-profile-1', role: Role.EMPLOYEE },
      { reason: 'Trip cancelled' },
    );

    expect(cancelled.status).toBe(RequestStatus.CANCELLED);
    expect(mockAuditLogs.some((l) => l.action === AuditAction.REQUEST_CANCELLED)).toBe(true);
  });
  ...
  it('25. Notification failure does not rollback transaction', async () => {
    notificationsService.sendNotification.mockRejectedValueOnce(new Error('FCM connection error'));
  ...
  it('26. Double approval attempt is rejected with BadRequestException', async () => {
    ...
    await expect(requestsService.approve(req.id, 'user-hr-admin')).rejects.toThrow(
      BadRequestException,
    );
  });
  ...
  it('27. Double rejection attempt is rejected with BadRequestException', async () => {
    ...
    await requestsService.reject(req.id, 'user-hr-admin', { reason: 'No staffing' });

    // Attempting second rejection
    await expect(
      requestsService.reject(req.id, 'user-hr-admin', { reason: 'No staffing 2' }),
    ).rejects.toThrow(BadRequestException);
  });

  it('28. Employee can retrieve own leave balance summary', async () => {
    const balances = await requestsService.getMyLeaveBalances('emp-profile-1', 2026);
  ...
  it('29. HR can create and adjust leave balance for employee', async () => {
    ...
    expect(mockAuditLogs.some((l) => l.action === AuditAction.LEAVE_BALANCE_UPDATED)).toBe(true);
  });
