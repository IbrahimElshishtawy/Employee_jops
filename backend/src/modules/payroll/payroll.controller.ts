...
export class PayrollController {
  ...
  @ApiOperation({ summary: 'HR: Set or update employee salary profile (with history versioning)' })
  setSalaryProfile(
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN, Role.HR_MANAGER)
  @ApiOperation({ summary: 'HR: View salary modification history for employee' })
  ...
  @ApiResponse({ status: 201, description: 'Advance request created successfully' })
  requestAdvance(
  ...
  @Get('advances/me')
  @ApiOperation({ summary: 'Employee: View my salary advance requests & installment schedule' })
  getMyAdvances(
    @CurrentUser('employeeProfileId') employeeProfileId: string,
  ...
  @Get('advances/:id')
  @ApiOperation({ summary: 'View detailed salary advance with installment schedule' })
  getAdvanceDetails(
    @Param('id') id: string,
    @CurrentUser() currentUser: any,
  ) {
  ...
  @ApiOperation({ summary: 'HR: Approve salary advance and generate installment schedule' })
  approveAdvance(
    @Param('id') id: string,
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN)
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'HR / Finance: Record payment towards an advance installment' })
  payInstallment(
    @Param('installmentId') installmentId: string,
    @CurrentUser('id') currentUserId: string,
    @Body() dto: PayInstallmentDto,
  ) {
    return this.payrollService.recordInstallmentPayment(installmentId, currentUserId, dto);
  }
  ...
  getPeriods(@Query('page') page?: number, @Query('limit') limit?: number) {
    return this.payrollService.getPayrollPeriods(Number(page) || 1, Number(limit) || 12);
  }
  ...
  calculatePeriod(
    ...
    return this.payrollService.calculatePeriodPayroll(periodId, dto, currentUserId);
  }
  ...
  finalizePeriod(
    ...
    return this.payrollService.finalizePayrollPeriod(periodId, currentUserId, dto);
  }
  ...
  @Roles(Role.SUPER_ADMIN, Role.HR_ADMIN)
  @ApiOperation({ summary: 'HR: Create post-finalization adjustment for payroll record' })
  adjustPayrollRecord(
    @Param('id') recordId: string,
    @CurrentUser('id') currentUserId: string,
    @Body() dto: CreateAdjustmentDto,
  ) {
    return this.payrollService.createPayrollAdjustment(recordId, currentUserId, dto);
  }
