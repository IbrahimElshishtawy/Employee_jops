...
export class EmployeesService {
        ...
        employeeProfile: {
          create: {
            ...
            isProfileComplete: Boolean(
              dto.firstName && dto.lastName && dto.workplaceId,
            ),
          },
        },
      },
  ...
  async completeProfile(userId: string, dto: CompleteProfileDto) {
    ...
    if (!workplace || !workplace.isActive) {
      throw new BadRequestException(
        "The selected workplace is invalid or inactive",
      );
    }
    ...
    if (dto.nationalId) {
      ...
      if (existingNationalId) {
        throw new ConflictException(
          "This National ID is already registered to another employee",
        );
      }
    }
  ...
  async getMyWorkplace(userId: string) {
    ...
    if (!user?.employeeProfile?.workplace) {
      throw new NotFoundException(
        "No active workplace assigned to this employee",
      );
    }
  ...
  async getMySchedule(userId: string) {
    ...
    if (!user?.employeeProfile?.schedule) {
      throw new NotFoundException(
        "No active work schedule assigned to this employee",
      );
    }
